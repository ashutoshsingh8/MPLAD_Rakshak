  ...
  return (
    <header className="bg-[#1f7a6b] text-white px-4 sm:px-6 py-2.5 flex items-center justify-between shadow-sm border-b border-[#186054] sticky top-0 z-50">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-sm sm:text-base font-black tracking-wider uppercase text-white">
              {t('da_portal.portal_title', 'DISTRICT AUTHORITY PORTAL')}
            </h1>
          ...
          <p className="text-[11px] text-teal-100/80 font-medium hidden sm:block">
            MPLAD Rakshak • {t('da_portal.portal_subtitle', 'Operational Scrutiny, Technical Sanctions & Anti-Fraud Workbench')}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={onRefresh}
          title={t('da_portal.refresh_data', 'Refresh live pipeline data')}
          ...
          {notificationsOpen && (
            <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-slate-200 text-slate-800 p-3 z-[60] animate-fade-in space-y-2">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-700">{t('da_portal.authority_notifications', 'Action Alerts')}</span>
                <span className="text-[10px] bg-red-100 text-red-700 px-1.5 py-0.5 rounded font-bold">{t('da_portal.urgent_badge', { count: 3, defaultValue: '3 Urgent' })}</span>
              </div>
              <div className="space-y-1.5 text-xs">
                <div className="p-2 rounded-lg bg-red-50 border border-red-200 text-red-800">
                  <div className="font-bold flex items-center gap-1">
                    <AlertTriangle className="w-3.5 h-3.5 text-red-600" />
                    <span>{t('da_portal.sla_breach_toast', '45-Day SLA Breached')}</span>
                  </div>
                  <p className="text-[11px] text-red-700 mt-0.5">{t('da_portal.sla_breached_desc', 'Khed PHC Sub-Center proposal pending for 52 days.')}</p>
                </div>
                <div className="p-2 rounded-lg bg-amber-50 border border-amber-200 text-amber-800">
                  <div className="font-bold">{t('da_portal.boq_inflation_alert', 'BOQ Inflation Warning')}</div>
                  <p className="text-[11px] text-amber-700 mt-0.5">{t('da_portal.boq_inflation_desc', 'Shirur Solar Grid quotes exceed SoR by +37.1%.')}</p>
                </div>
                <div className="p-2 rounded-lg bg-purple-50 border border-purple-200 text-purple-800">
                  <div className="font-bold">{t('da_portal.exif_tamper_alert', 'EXIF Tamper Detected')}</div>
                  <p className="text-[11px] text-purple-700 mt-0.5">{t('da_portal.exif_tamper_desc', 'Chakan photo displaced 3.4km from official site.')}</p>
                </div>
              </div>
            </div>
          )}
        ...
        {onExitToPublic && (
          ...
          >
            ...
            <span>{t('nav.exit_to_public', 'Exit to Public')}</span>
          </button>
        )}
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="hidden sm:block text-left">
              ...
              <p className="text-[10px] text-teal-100/75 leading-none">{localizedDistrict}</p>
            </div>
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-200 text-slate-800 py-2 z-[60] animate-fade-in text-xs">
              <div className="px-4 py-2 border-b border-slate-100">
                ...
                <p className="text-[11px] text-slate-500">{t('da_portal.collector_designation', daOfficerProfile.designation)}</p>
                <p className="text-[10px] font-mono text-emerald-700 mt-0.5">{localizedDistrict}</p>
              </div>
